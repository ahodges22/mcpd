...
On POSIX, request a new process group in the child pre-exec path and confirm from the parent that `getpgid(helperPID) == helperPID`; abort before sending a request if the helper is not its own group leader. Record a restrictive, atomic `native-helper.json` marker only after this confirmation. The marker contains no secret. It contains `phase=in-flight`, the operation deadline, the later termination-attempt deadline, the owner PID/start time, canonical helper executable path, hidden-helper identifier, helper PID, confirmed process-group identity, process start time, and daemon/CLI instance identifier. Then send the request through the pipe.
...
Before sending any signal, prove that the target is an mcpd native helper. Require PID and start-time match, canonical executable-path match, the expected hidden-helper argument, and the marker's random instance identifier in the target command line. Before a group-directed signal, also require the recorded PGID equals the helper PID, the verified leader still owns that PGID, the PGID differs from the signaling process's own group, and every enumerated group member carries the matching non-secret instance tag. If the leader is gone or any group member cannot be positively tied to the instance, do not signal the group. Signal only individually identity-proven members, otherwise retain wedged state until they exit naturally. If any identity check fails, treat the marker as stale, remove it durably, and send no signal.
