...
- [x] 7.2 Run the complete real-session acceptance matrix on macOS and Linux, including healthy, attention, offline recovery, reconnect, OAuth browser launch, dashboard launch, duplicate action suppression, clean Quit, crash restart, login start, no macOS Dock icon, host absence, and late host appearance; then run adversarial code review and resolve every merge-blocking finding. Proof: the signed-off platform matrix and review result in `verification.md`. Commit: `test: complete tray acceptance review`.
