...
- [ ] 3.1 Implement the no-proxy loopback status client with bounded bodies, a two-second poll budget, minimal response types, and refusal of non-loopback addresses. Proof: `TestClientStatus`, `TestClientStatusErrors`, and `TestClientRejectsNonLoopback` under `go test ./internal/tray`. Commit: `feat: add tray status client`.
